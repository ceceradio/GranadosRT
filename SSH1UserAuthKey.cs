...
namespace GranadosRT.Routrek.SSHCV1
{
	...
	/// </summary>
	public sealed class SSH1UserAuthKey
	{
		...
		internal BigInteger PublicModulus {
			get {
				return _modulus;
			}
		}
		...
		internal BigInteger decryptChallenge(BigInteger encryptedchallenge) {
		...
		private static byte[] ConvertToKey(string passphrase) {
			byte[] t = Encoding.UTF8.GetBytes(passphrase);
            byte[] md5 = HashAlgorithmProvider.OpenAlgorithm(HashAlgorithmNames.Md5).HashData(t.AsBuffer()).ToArray();
