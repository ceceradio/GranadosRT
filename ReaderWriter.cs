...
namespace GranadosRT.Routrek.SSHC {
	...
	internal abstract class SSHDataReader {
        ...
        {
            if (_offset + 1 >= _data.Length) throw new IOException(Strings.GetString("UnexpectedEOF"));

            int ret = (((int)_data[_offset]) << 8) + _data[_offset + 1];
		...
		*/
        ...
        {
            if (_offset + delta >= _data.Length || _offset + delta < 0)
            {
	...
	internal abstract class SSHDataWriter : IKeyWriter {
		...
		public void Write(string data) {
            ...
            if (data.Length > 0) Write(dataArray);
		}
...
namespace GranadosRT.Routrek.SSHCV1 {
	...
	internal class SSH1DataReader : GranadosRT.Routrek.SSHC.SSHDataReader {
	...
	internal class SSH1DataWriter : GranadosRT.Routrek.SSHC.SSHDataWriter {
...
namespace GranadosRT.Routrek.SSHCV2 {
	...
	internal class SSH2DataReader : GranadosRT.Routrek.SSHC.SSHDataReader {
	...
	internal class SSH2DataWriter : GranadosRT.Routrek.SSHC.SSHDataWriter {
