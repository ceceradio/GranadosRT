...
namespace GranadosRT.Routrek.SSHCV1
{
		...
		*/
		internal void ConstructAndCheck(byte[] buf, int packet_length, int padding_length, bool check_crc) {
			...
			if(check_crc) {
                ...
                if (_CRC != c)
                    //throw new Exception("CRC Error", buf);
	...
	internal interface ISSH1PacketHandler : IHandlerBase {
		//Task OnPacket(SSH1Packet packet);
        void OnPacket(SSH1Packet packet);
	}

	internal class SynchronizedSSH1PacketHandler : ISSH1PacketHandler {
		internal List<object> _packets;
		...
		internal SynchronizedSSH1PacketHandler() {
			...
			_packets = new List<object>();
		}

		//public Task OnPacket(SSH1Packet packet) {
	...
	internal class CallbackSSH1PacketHandler : ISSH1PacketHandler {
		...
		//public async Task OnPacket(SSH1Packet packet) {
        public void OnPacket(SSH1Packet packet)
        {
			//await _connection.AsyncReceivePacket(packet);
